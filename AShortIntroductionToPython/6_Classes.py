...
#   attributes to the "self" object as follows:
        self.name = name
        self.grade = grade
#   Now both name and grade are parts of the student object. We could also make a variable that stores the time this student object was created:
        self.__created = time()
...
#   case the entire name. The -1 represents the step size, - indicates that we want the string backwards, and 1 indicates that we want every letter. A 2 would give us every other letter, 3
#   every third letter, etc.
#       Now we have defined our class, lets see how we would use it.

if __name__ == "__main__":
#   Recall this snippet from file 1 means that this bit of code is only executed if the file is executed directly, not if we import this file as a module in another program.
    s = Student("Billy Smith", 9)
#   We have created a new student called s, with the name Billy Smith, and the grade 9. Notice that the __init__ function is called just with the name of the class itself.
